...
limpiar_timer0 macro
    ...
    bcf ((INTCON) and 07Fh), 2 ;Se baja la bandera de overflow del timer0
    endm
...
interrupcion:
    btfsc ((INTCON) and 07Fh), 2 ;Reviso si la bandera de TMR0 se enciende
    call int_timer0 ;Ir a interrupcion

    btfsc ((PIR1) and 07Fh), 0 ;Revisar si hay overflow en timer1
...
conf_reloj:
    banksel OSCCON
    bcf ((OSCCON) and 07Fh), 4 ;el oscilador interno esta a una frecuencia de 1MHz
    bcf ((OSCCON) and 07Fh), 5
    bsf ((OSCCON) and 07Fh), 6
    bsf ((OSCCON) and 07Fh), 0 ;Utilizamos reloj interno
    return


conf_timer0:
    banksel TRISA
    bcf ((OPTION_REG) and 07Fh), 5 ;ciclo de intruccion FOSC/4
    bcf ((OPTION_REG) and 07Fh), 3 ;El preescalar se asigna a timer0
    bsf ((OPTION_REG) and 07Fh), 0 ;valor del preescalar de seleccion de bits 1:256
...
conf_timer1:
    ...
    bsf ((T1CON) and 07Fh), 4 ;preesccalar 1:8
...
conf_int_Enbale:
    ...
    bsf ((INTCON) and 07Fh), 5 ;Se habilita la interrupcion del TMR0
    bcf ((INTCON) and 07Fh), 2 ;Bajamos la bandera de TMR0
